...
def details(request):
    try:
        ...
        b = True
    a = Hospital.objects.raw('''SELECT id AS idnum, name AS name,  CONCAT(address, ' ', city, ' ', state, ' ', zipcode) AS address, phonenumber AS phonenumber, hospitaltype AS hospitaltype, emergency AS emergency, criteria AS meetcriteria, overallrating AS overallrating, mortalitycomparison AS mortality comparison, safetycomparison AS safetycomparison, readmissioncomparison AS readmissioncomparison, patientexperiencecomparison AS experiencecomparison, effectivenesscomparison AS effectivenesscomparison, timelinesscomparison AS timelinesscomparison, efficientimagingcomparison AS medicalimagingcomparison FROM general WHERE id = .key''')
    hospital_info = []
